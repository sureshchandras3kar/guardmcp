...
def register(mcp: FastMCP, ctx: ToolContext) -> None:
    ...
    async def guardmcp_setup(answers: JsonDict = None) -> str:
        ...
        # Bug fix: mode="readwrite" must actually grant write actions — this
        # was previously hardcoded to read_actions regardless of `mode`, so a
        # brand-new user picking "readwrite" got a policy that silently
        # behaved like readonly until they hand-edited the generated YAML.
        allowed_actions = list(read_actions)
        if mode == "readwrite":
            allowed_actions += sorted(a.value for a in WRITE_ACTIONS)
        policy_doc["actions"] = {"allow": allowed_actions}
        if mask_list:
