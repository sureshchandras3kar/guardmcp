...
from .schema import apply_mask, build_field_stats, build_type_map, infer_schema
...
class MongoSchemaCache:
    def __init__(
        ...
        # M3: bounded LRU cache: (database, collection) → (raw_schema, type_map, raw_stats, expiry).
        ...
        # canonical BSON type map consumed by the filter marshaller; raw_stats is
        # the per-field sample stats (mask applied at read time). All are derived
        # from the SAME document sample so they never disagree.
        ...
        self._schema_cache: OrderedDict[
            tuple[str | None, str], tuple[dict, dict, dict, float]
        ] = OrderedDict()
    ...
    async def _sample_schema(
        ...
        if self._schema_cache_ttl > 0:
            ...
            if cached is not None:
                raw_schema, type_map, _raw_stats, expiry = cached
        ...
        raw_stats = build_field_stats(docs, [])
    ...
    async def field_stats(
        ...
        never carry values."""
        await self._sample_schema(collection, sample_size, database=database)
        cached = self._schema_cache.get((database, collection))
        if cached is not None:
            raw_stats = cached[2]
        else:
            # caching disabled (ttl<=0) or evicted: sample directly so stats are
            # never silently empty. Uses the same bounded sample size.
            col = self._client.get_collection(collection, database)
            n = sample_size or self._schema_sample
            docs = await col.find({}).limit(n).to_list(n)
            raw_stats = build_field_stats(docs, [])
