"""
PostgreSQL-specific request validation.

Backend-specific guard knowledge lives in the plugin (mirroring the MongoDB
plugin's guard.py). These checks run BEFORE translation/execution and reject
anything that cannot be expressed as safe, parameterized SQL.

The guard logic is shared with MySQL in :mod:`plugins.sql.validate_base`; this
module is a thin shim that binds the PostgreSQL ``quote_ident`` (double-quoted
